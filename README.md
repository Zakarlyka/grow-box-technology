...
**Крок 1: Генерація deviceId та QR-коду**
1. Відкрийте Dashboard на сайті
2. Натисніть кнопку **"Підключити новий"**
3. Система автоматично згенерує унікальний deviceId (формат: ESP-XXXXXXXXX-XXXXXX)
4. З'явиться діалогове вікно з QR-кодом та інструкціями

**Крок 2: Налаштування ESP8266**
1. Скануйте QR-код телефоном - отримаєте посилання виду `http://192.168.4.1/?deviceId=ESP-MGHXYMP5-GV9YOC`
2. Запам'ятайте або скопіюйте згенерований deviceId з QR-коду
3. Підключіться до WiFi точки доступу **"GrowBox-Setup"** (пароль: 12345678)
4. Відкрийте браузер та перейдіть за адресою **http://192.168.4.1**
5. На порталі ESP8266:
   - Виберіть вашу домашню WiFi мережу зі списку
   - Введіть пароль від WiFi
   - **Важливо:** Введіть отриманий deviceId у відповідне поле
   - Натисніть кнопку "Connect"
6. Дочекайтеся підключення ESP8266 до вашої WiFi мережі

**Крок 3: Підтвердження на сайті**
1. Поверніться на сайт (діалогове вікно з QR-кодом повинно бути ще відкрите)
2. Натисніть кнопку **"Підтвердити підключення"** під QR-кодом
3. Після успішного підтвердження з'явиться сповіщення "Підключення підтверджено!"
4. Пристрій автоматично з'явиться у списку на Dashboard зі статусом "online"

**Крок 4: Перевірка статусу**
1. На сторінці Dashboard знайдіть блок "Статус підключення"
2. Переконайтеся, що кількість онлайн пристроїв збільшилася
3. Ваш новий пристрій буде відображатися зі статусом "online" та поточним часом підключення
